...
const slides: Slide[] = [
    // Slide 1: Title / Hero
    {
        ...
        subtitle: 'A centralized system uniting Slots, Vehicles, Visitors, and Billing.',
        gradient: 'from-primary-500/10 via-transparent to-emerald-500/10',
        accentColor: 'primary',
        content: {
            type: 'hero',
            tagline: 'From Vehicle Registration → RFID Issue → Visitor Entry → Billing — completely streamlined.',
            highlights: [
                { icon: 'i-lucide-map', label: 'Parking Slots', value: 'Dynamic' },
                { icon: 'i-lucide-car', label: 'Resident Vehicles', value: 'Centralized' },
                { icon: 'i-lucide-users', label: 'Visitor Validations', value: 'Seamless' },
                { icon: 'i-lucide-receipt', label: 'Billing Records', value: 'Real-time' }
            ]
        }
    },
    // Slide 2: Platform Architecture
    {
        ...
        subtitle: 'A modern, reactive architecture using Vue 3 Composition API and Pinia stores.',
        ...
        content: {
            ...
            layers: [
                ...
                },
                {
                    ...
                    items: ['Role-Based Access Control', 'Business Rules Engine', 'Lifecycle State Machines'],
                    color: 'warning'
                },
                {
                    icon: 'i-lucide-database',
                    title: 'Domain Stores & State',
                    items: ['Pinia Stores (Slots, Vehicles, Sessions)', 'Real-time Cross-Module Reactivity', 'Centralized State Management'],
                    color: 'success'
                }
            ]
        }
    ...
    // Slide 3: Core Workflow
    {
        ...
        title: 'Vehicle Access & Flow',
        subtitle: 'A seamless workflow connecting resident allocations and visitor reservations.',
        gradient: 'from-amber-500/10 via-transparent to-orange-500/10',
        accentColor: 'warning',
        content: {
            type: 'workflow',
            steps: [
                { icon: 'i-lucide-car', title: 'Vehicle Registration', description: 'Resident registers vehicle and uploads documents.', role: 'Resident' },
                { icon: 'i-lucide-tag', title: 'RFID Assignment', description: 'Admin issues an active RFID tag.', role: 'Admin' },
                { icon: 'i-lucide-map-pin', title: 'Slot Allocation', description: 'System allocates a dedicated or shared slot.', role: 'Operations' },
                { icon: 'i-lucide-qr-code', title: 'Visitor Reservation', description: 'Resident pre-registers a visitor generating a QR code.', role: 'Resident' },
                { icon: 'i-lucide-check-circle', title: 'Front Desk Validation', description: 'Attendant scans QR code to start active session.', role: 'Front Desk' },
                { icon: 'i-lucide-credit-card', title: 'Payment & Exit', description: 'Transient fees are computed based on session time before exit.', role: 'Finance' }
            ]
        }
    },
    // Slide 4: Reports Dashboard
    {
        id: 'reports',
        badge: 'Analytics',
        badgeIcon: 'i-lucide-bar-chart-3',
        title: 'Reports Dashboard',
        subtitle: 'Real-time insights and analytical reporting for parking operations.',
        gradient: 'from-emerald-500/10 via-transparent to-teal-500/10',
        accentColor: 'success',
        content: {
            type: 'features',
            items: [
                { icon: 'i-lucide-car', title: 'Vehicle & RFID Inventory', description: 'Track all registered resident vehicles and active RFID tags.', color: 'primary' },
                { icon: 'i-lucide-pie-chart', title: 'Slot Occupancy Trends', description: 'Monitor daily and hourly utilization of parking spaces.', color: 'warning' },
                { icon: 'i-lucide-receipt', title: 'Transaction Summaries', description: 'Daily and monthly aggregation of collected parking fees.', color: 'success' },
                { icon: 'i-lucide-alert-triangle', title: 'Violations & Overstay', description: 'Automatically flag vehicles exceeding allotted parking time.', color: 'error' }
            ]
        }
    },
    // Slide 5: Role-Based Access Control
    {
        ...
        content: {
            type: 'roles',
            roles: [
                { icon: 'i-lucide-shield-alert', name: 'Admin', permissions: ['Platform Operations', 'Users & Roles Management', 'Global Policies'], color: 'purple' },
                { icon: 'i-lucide-briefcase', name: 'Parking Operations', permissions: ['Slot Allocation', 'RFID Issuance', 'Vehicle Approvals'], color: 'blue' },
                { icon: 'i-lucide-users', name: 'Front Desk Personnel', permissions: ['Visitor Validation', 'Walk-in Entry Logs', 'Manual Verification'], color: 'amber' },
                { icon: 'i-lucide-receipt', name: 'Finance Staff', permissions: ['Ticket Payments', 'Resident Charges', 'Revenue Reports'], color: 'emerald' }
            ]
        }
    },
    // Slide 6: Slot Management
    {
        id: 'slot-admin',
        badge: 'Management',
        badgeIcon: 'i-lucide-map',
        title: 'Slot & Allocation Management',
        subtitle: 'Efficient administration of parking spaces and resident allocations.',
        gradient: 'from-blue-500/10 via-transparent to-indigo-500/10',
        accentColor: 'primary',
        content: {
            type: 'features',
            items: [
                { icon: 'i-lucide-list', title: 'Slot Directory', description: 'Master list of all facility parking spaces.', color: 'secondary' },
                { icon: 'i-lucide-link', title: 'Resident Allocations', description: 'Manage dedicated and shared slot assignments.', color: 'warning' },
                { icon: 'i-lucide-activity', title: 'Real-Time Availability', description: 'Monitor live occupancy of individual slots.', color: 'primary' },
                { icon: 'i-lucide-repeat', title: 'Allocation Reassignment', description: 'Seamlessly transfer slots between resident vehicles.', color: 'error' }
            ]
        }
    },
    // Slide 7: Tech Stack
            ...
            categories: [
                {
                    ...
                    items: [
                        ...
                        { name: 'TypeScript', description: 'Strict typing for robust domain models (Slots, Vehicles)' }
                    ],
                    color: 'success'
                },
                {
    ...
    // Slide 8: Key Metrics
    {
        ...
        content: {
            type: 'stats',
            metrics: [
                { icon: 'i-lucide-layout-dashboard', label: 'Module Pages', value: '18+', description: 'Specialized dashboard views', color: 'primary' },
                ...
                { icon: 'i-lucide-tag', label: 'RFID Tags', value: '500+', description: 'Monitored endpoints', color: 'error' },
    ...
    // Slide 9: Closing
    {
        ...
        subtitle: 'A complete platform showcasing modern parking management capabilities.',
        gradient: 'from-primary-500/10 via-emerald-500/5 to-amber-500/10',
        accentColor: 'primary',
        content: {
            type: 'closing',
            bullets: [
                { icon: 'i-lucide-check-circle', text: 'Unified platform for Operations, Front Desk, and Finance' },
                { icon: 'i-lucide-check-circle', text: 'Centralized management of resident vehicles and RFIDs' },
                { icon: 'i-lucide-check-circle', text: 'Seamless visitor reservation and validation workflows' },
        ...
        <div class="group relative z-50 shrink-0">
            ...
            <div class="absolute top-0 left-0 right-0 h-12 flex items-center justify-between px-6 border-b border-neutral-200/50 dark:border-neutral-800/50 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md transition-all duration-300 -translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100 z-50">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-bold text-neutral-900 dark:text-white">Parking<span class="text-primary">System</span></span>
